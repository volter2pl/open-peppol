<?php

namespace Volter\OpenPeppol\Dict;

/**
 * Class EAS
 * @package Volter\OpenPeppol\Dict
 * @link https://test-docs.peppol.eu/poacc/upgrade-3/codelist/eas/
 */
class EAS
{
    /**
     * System Information et Repertoire des Entreprise et des Etablissements: SIRENE
     */
    const CODE_0002 = '0002';

    /**
     * Organisationsnummer (Swedish legal entities)
     */
    const CODE_0007 = '0007';

    /**
     * SIRET-CODE
     */
    const CODE_0009 = '0009';

    /**
     * LY-tunnus
     */
    const CODE_0037 = '0037';

    /**
     * Data Universal Numbering System (D-U-N-S Number)
     */
    const CODE_0060 = '0060';

    /**
     * EAN Location Code
     */
    const CODE_0088 = '0088';

    /**
     * DANISH CHAMBER OF COMMERCE Scheme (EDIRA compliant)
     */
    const CODE_0096 = '0096';

    /**
     * FTI - Ediforum Italia, (EDIRA compliant)
     */
    const CODE_0097 = '0097';

    /**
     * Vereniging van Kamers van Koophandel en Fabrieken in Nederland (Association of Chambers of Commerce and Industry in the Netherlands), Scheme (EDIRA compliant)
     */
    const CODE_0106 = '0106';

    /**
     * Directorates of the European Commission
     */
    const CODE_0130 = '0130';

    /**
     * SIA Object Identifiers
     */
    const CODE_0135 = '0135';

    /**
     * SECETI Object Identifiers
     */
    const CODE_0142 = '0142';

    /**
     * Australian Business Number (ABN) Scheme
     */
    const CODE_0151 = '0151';

    /**
     * Numéro d'identification suisse des enterprises (IDE), Swiss Unique Business Identification Number (UIDB)
     */
    const CODE_0183 = '0183';

    /**
     * DIGSTORG
     */
    const CODE_0184 = '0184';

    /**
     * Dutch Originator's Identification Number
     */
    const CODE_0190 = '0190';

    /**
     * Centre of Registers and Information Systems of the Ministry of Justice
     */
    const CODE_0191 = '0191';

    /**
     * Enhetsregisteret ved Bronnoysundregisterne
     */
    const CODE_0192 = '0192';

    /**
     * UBL.BE party identifier
     */
    const CODE_0193 = '0193';

    /**
     * Singapore UEN identifier
     */
    const CODE_0195 = '0195';

    /**
     * Kennitala - Iceland legal id for individuals and legal entities
     */
    const CODE_0196 = '0196';

    /**
     * ERSTORG
     */
    const CODE_0198 = '0198';

    /**
     * Legal Entity Identifier (LEI)
     */
    const CODE_0199 = '0199';

    /**
     * Legal entity code (Lithuania)
     */
    const CODE_0200 = '0200';

    /**
     * Codice Univoco Unità Organizzativa iPA
     */
    const CODE_0201 = '0201';

    /**
     * Indirizzo di Posta Elettronica Certificata
     */
    const CODE_0202 = '0202';

    /**
     * Leitweg-ID
     */
    const CODE_0204 = '0204';

    /**
     * Danish Ministry of the Interior and Health
     */
    const CODE_9901 = '9901';

    /**
     * Ufficio responsabile gestione partite IVA
     */
    const CODE_9906 = '9906';

    /**
     * TAX Authority
     */
    const CODE_9907 = '9907';

    /**
     * Hungary VAT number
     */
    const CODE_9910 = '9910';

    /**
     * Business Registers Network
     */
    const CODE_9913 = '9913';

    /**
     * Österreichische Umsatzsteuer-Identifikationsnummer
     */
    const CODE_9914 = '9914';

    /**
     * Österreichisches Verwaltungs bzw. Organisationskennzeichen
     */
    const CODE_9915 = '9915';

    /**
     * SOCIETY FOR WORLDWIDE INTERBANK FINANCIAL, TELECOMMUNICATION S.W.I.F.T
     */
    const CODE_9918 = '9918';

    /**
     * Kennziffer des Unternehmensregisters
     */
    const CODE_9919 = '9919';

    /**
     * Agencia Española de Administración Tributaria
     */
    const CODE_9920 = '9920';

    /**
     * Andorra VAT number
     */
    const CODE_9922 = '9922';

    /**
     * Albania VAT number
     */
    const CODE_9923 = '9923';

    /**
     * Bosnia and Herzegovina VAT number
     */
    const CODE_9924 = '9924';

    /**
     * Belgium VAT number
     */
    const CODE_9925 = '9925';

    /**
     * Bulgaria VAT number
     */
    const CODE_9926 = '9926';

    /**
     * Switzerland VAT number
     */
    const CODE_9927 = '9927';

    /**
     * Cyprus VAT number
     */
    const CODE_9928 = '9928';

    /**
     * Czech Republic VAT number
     */
    const CODE_9929 = '9929';

    /**
     * Germany VAT number
     */
    const CODE_9930 = '9930';

    /**
     * Estonia VAT number
     */
    const CODE_9931 = '9931';

    /**
     * United Kingdom VAT number
     */
    const CODE_9932 = '9932';

    /**
     * Greece VAT number
     */
    const CODE_9933 = '9933';

    /**
     * Croatia VAT number
     */
    const CODE_9934 = '9934';

    /**
     * Ireland VAT number
     */
    const CODE_9935 = '9935';

    /**
     * Liechtenstein VAT number
     */
    const CODE_9936 = '9936';

    /**
     * Lithuania VAT number
     */
    const CODE_9937 = '9937';

    /**
     * Luxemburg VAT number
     */
    const CODE_9938 = '9938';

    /**
     * Latvia VAT number
     */
    const CODE_9939 = '9939';

    /**
     * Monaco VAT number
     */
    const CODE_9940 = '9940';

    /**
     * Montenegro VAT number
     */
    const CODE_9941 = '9941';

    /**
     * Macedonia, the former Yugoslav Republic of VAT number
     */
    const CODE_9942 = '9942';

    /**
     * Malta VAT number
     */
    const CODE_9943 = '9943';

    /**
     * Netherlands VAT number
     */
    const CODE_9944 = '9944';

    /**
     * Poland VAT number
     */
    const CODE_9945 = '9945';

    /**
     * Portugal VAT number
     */
    const CODE_9946 = '9946';

    /**
     * Romania VAT number
     */
    const CODE_9947 = '9947';

    /**
     * Serbia VAT number
     */
    const CODE_9948 = '9948';

    /**
     * Slovenia VAT number
     */
    const CODE_9949 = '9949';

    /**
     * Slovakia VAT number
     */
    const CODE_9950 = '9950';

    /**
     * San Marino VAT number
     */
    const CODE_9951 = '9951';

    /**
     * Turkey VAT number
     */
    const CODE_9952 = '9952';

    /**
     * Holy See (Vatican City State) VAT number
     */
    const CODE_9953 = '9953';

    /**
     * Swedish VAT number
     */
    const CODE_9955 = '9955';

    /**
     * Belgian Crossroad Bank of Enterprises
     */
    const CODE_9956 = '9956';

    /**
     * French VAT number
     */
    const CODE_9957 = '9957';

    /**
     * German Leitweg ID
     */
    const CODE_9958 = '9958';

    public static function verify($code)
    {
        if (!in_array($code, [
            self::CODE_0002, self::CODE_0007, self::CODE_0009, self::CODE_0037, self::CODE_0060, self::CODE_0088,
            self::CODE_0096, self::CODE_0097, self::CODE_0106, self::CODE_0130, self::CODE_0135, self::CODE_0142,
            self::CODE_0151, self::CODE_0183, self::CODE_0184, self::CODE_0190, self::CODE_0191, self::CODE_0192,
            self::CODE_0193, self::CODE_0195, self::CODE_0196, self::CODE_0198, self::CODE_0199, self::CODE_0200,
            self::CODE_0201, self::CODE_0202, self::CODE_0204, self::CODE_9901, self::CODE_9906, self::CODE_9907,
            self::CODE_9910, self::CODE_9913, self::CODE_9914, self::CODE_9915, self::CODE_9918, self::CODE_9919,
            self::CODE_9920, self::CODE_9922, self::CODE_9923, self::CODE_9924, self::CODE_9925, self::CODE_9926,
            self::CODE_9927, self::CODE_9928, self::CODE_9929, self::CODE_9930, self::CODE_9931, self::CODE_9932,
            self::CODE_9933, self::CODE_9934, self::CODE_9935, self::CODE_9936, self::CODE_9937, self::CODE_9938,
            self::CODE_9939, self::CODE_9940, self::CODE_9941, self::CODE_9942, self::CODE_9943, self::CODE_9944,
            self::CODE_9945, self::CODE_9946, self::CODE_9947, self::CODE_9948, self::CODE_9949, self::CODE_9950,
            self::CODE_9951, self::CODE_9952, self::CODE_9953, self::CODE_9955, self::CODE_9956, self::CODE_9957,
            self::CODE_9958
        ])) {
            throw new \InvalidArgumentException("Value MUST be part of code list EAS");
        }
    }
}
